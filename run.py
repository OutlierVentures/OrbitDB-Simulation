...
def run_loop(it,x_list,x_list_bloom,x_list_hybrid_bloom,x_list_dag_height,limit,parameters):
    ...
    env = SimulationManager(peers, spokes, stats,time_limit=limit, broadcast=True,
                            ...
                            event_type=parameters[3],message_loss=parameters[4],hash_count=parameters[5],filter_size=parameters[6])
    ...
    _processes = []
    for s in states:
        p = multiprocessing.Process(target=start_it,args=(s,))
        p.daemon = False
        _processes.append(p)
        p.start()
        start_it(s)

    for pa in _processes:
        pa.join()
        ...
        with multiprocessing.Manager() as manager:
            ...
            while i <= 128:
                p = multiprocessing.Process(target=run_loop,args=(i,x_list,x_list_bloom,x_list_hybrid_bloom,
                                                                  x_list_dag_height,limit,params))
                processes.append(p)
                p.start()
                i *= 2
...
if __name__ == '__main__':
    run([60, 'skewed', 'random', 'skewed', False, 2, 500])
